...
class Block(pg.sprite.Sprite):
    ...
    def is_collide(self, pos):
        ...
        if 0 <= x < FIELD_W and y < FIELD_H and (
            y < 0 or not self.tetramino.tetris.field_array[y][x]
        ):
            return False
        return True
...
class Tetramino:
    def __init__(self, tetris, current=True):
        self.tetris = tetris
        self.shape = random.choices(list(TETRAMINOS.keys()), weights=[
                                    10, 10, 10, 10, 10, 10, 10, 5, 5, 10, 10], k=1)
        self.image = random.choice(tetris.app.images)

        self.blocks = [Block(self, pos) for pos in TETRAMINOS[self.shape[0]]]
        self.landing = False
        self.current = current

    def rotate(self):
        if self.shape in ("O", ".", "0"):
            return
